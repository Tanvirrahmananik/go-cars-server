...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.apng3.mongodb.net/myFirstDatabase?retryWrites=true&w=majority`;
...
client.connect((err) =>{
    const servicesCollection = client.db("go-cars").collection('services')
    const bookingCollection = client.db("go-cars").collection('bookings')
...
app.get('/', (req, res) => {
    res.send('go-cars server')
})

app.listen(port, () => {
    console.log('go-cars server on port', port)
})